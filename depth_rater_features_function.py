...
def word2vec_similarity(convo, model):
    ...
    if convo_vec == None:
        convo_vec = numpy.array([[0.0 for x in range(0, 100)]])
    #pprint.pprint(sum(convo_vec).tolist())
    return map(lambda x : x / (len(convo) + 1), similarity_sums) + sum(convo_vec).tolist()
...
def get_convolist(log_root):
    ...
    for root, subdirs, files in walk(log_root):
        ...
        #                count += 1
            ...
            conv.append(extract_convo(join(root,f)))
            feature.append( extract_features(extract_convo(join(root,f))))
...
if __name__ == "__main__":
    convs, features = get_convolist('/home/ubuntu/zhou/Backend/rating_log/v5')
    depth_value =[]
    X_train_org, scaler_depth, clf_depth = pickle.load(open('depth_rater.pkl'))
    for feature in features:
        feature_test = scaler_depth.transform(feature)
        depth_value.append(clf_depth.predict(feature_test))
    word_set_value = []
    for conv in convs:
        word_set =[]
        for utt in conv:
            print utt
            utt2 = utt['TickTock'] + utt['You']
            tokens = nltk.word_tokenize(utt2)
            word_set = word_set+tokens
        word_set = list(set(word_set))
        info_gain = len(word_set)
        word_set_value.append(info_gain)
    print depth_value
    print sum(depth_value)/float(len(depth_value))
    print sum(word_set_value)/float(len(word_set_value))
